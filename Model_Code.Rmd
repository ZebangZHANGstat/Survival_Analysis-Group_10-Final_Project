...
Because accelerated failure time models do not accommodate time-varying coefficients, the AFT analysis was performed using the same baseline and interaction covariates as the Cox model, but assuming time-invariant effects. This provided a sensitivity comparison to assess whether the added complexity of the extended Cox model was supported by improvements in model fit.

```{r}

 
aft_weibull= survreg(
  Surv(overall_survival_months, overall_survival_status) ~ 
    age_at_diagnosis +
    her2_status +
    hormone_therapy +
    ...
    radio_therapy +
    log(tumor_size) +
    log(tumor_size):hormone_therapy +
...
The AFT model with the lowest AIC had a weibul distribution. 
